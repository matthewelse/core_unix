...
CAMLprim value core_linux_sendfile_stub(value v_sock, value v_fd, value v_pos,
                                        value v_len) {
...
CAMLprim value core_linux_gettcpopt_string_stub(value v_socket, value v_optname) {
  ...
  /* There are so few sockopts that take string buffers as values that I'm
     scared of trying to write generic code to handle them right now. It's not
     obvious that they will have the same semantics as to where they place their
     null bytes or what the maximum option length will be.

     Python, for example, has a generic implementation, but it leads to slightly
     ...
     For now, we just handle each option individually, and cite the relevant
     docs for how they should be treated. */

  switch (optname) {
  case TCP_CONGESTION: {
    /* man 7 tcp says: optlen specifies the amount of space available in the
       buffer pointed to by optval, which should be at least 16 bytes
       (TCP_CA_NAME_MAX). On return, the buffer pointed to by optval is set to a
       null-terminated string containing the name of the congestion-control
       algorithm for this socket, and optlen is set to the minimum of its
       original value and TCP_CA_NAME_MAX. */
...
CAMLprim value core_linux_settcpopt_string_stub(value v_socket, value v_optname,
                                                value v_optval) {
  ...
  switch (optname) {
  case TCP_CONGESTION: {
    /* man 7 tcp says: optlen specifies the length of the congestion-control
       algorithm name contained in the buffer pointed to by optval; this length
       need not include any terminating null byte. */
...
CAMLprim value core_linux_send_no_sigpipe_stub(value v_fd, value v_pos, value v_len,
  ...
  if (ret == -1)
    uerror("send_no_sigpipe", Nothing);
  return Val_long(ret);
}
...
CAMLprim value core_linux_pr_set_pdeathsig_stub(value v_sig) {
  ...
  if (prctl(PR_SET_PDEATHSIG, sig) == -1)
    uerror("pr_set_pdeathsig", Nothing);
  return Val_unit;
}
...
CAMLprim value core_linux_pr_get_pdeathsig_stub(value __unused v_unit) {
  ...
  if (prctl(PR_GET_PDEATHSIG, &sig) == -1)
    uerror("pr_get_pdeathsig", Nothing);
...
CAMLprim value core_linux_getpriority(value v_tid) {
  ...
  /* As [man getpriority] states, since getpriority() can legitimately return
    the value -1, it is necessary to clear errno prior to the call, then check
    it afterwards to determine if [priority == -1] is an error or legitimate
    value. */
  old_errno = errno;
  errno = 0;
...
CAMLprim value core_linux_get_mac_address(value v_interface) {
  ...
  if (error == NULL) {
    ...
    /* While [ether_ntoa] seems promising, it has an unusual interpretation of
 ...
 * for security reasons. Taking a string argument, it does not fit the sockopt
 * stubs used for other socket options.
 */
...
CAMLprim value core_linux_epoll_wait(value v_epfd, value v_array, value v_timeout) {
  ...
  /* [CAMLparam1] is needed here to ensure that the bigstring does not get
     finalized during the period when we release the Caml lock, below.
  */
  CAMLparam1(v_array);
...
CAMLprim value core_linux_timerfd_gettime(value v_fd) {
  ...
  if (retcode == -1)
    uerror("timerfd_gettime", Nothing);
...
#endif
#if !defined(MDF_HUGE_2MB)
#define MFD_HUGE_2MB (21U << 26)
#endif
#if !defined(MDF_HUGE_1GB)
...
MEMFD_INT63(MFD_ALLOW_SEALING)
MEMFD_INT63(MFD_HUGETLB)
MEMFD_INT63(MFD_NOEXEC_SEAL)
MEMFD_INT63(MFD_EXEC)
MEMFD_INT63(MFD_HUGE_2MB)
MEMFD_INT63(MFD_HUGE_1GB)

CAMLprim value core_linux_memfd_create(value v_flags, value v_initial_size,
                                       value v_name) {
  int fd = memfd_create(String_val(v_name), Int63_val(v_flags));
  if (fd == -1) {
...
CAMLprim value core_linux_eventfd(value v_initval, value v_flags) {
  ...
  if (initval < 0)
    caml_failwith("eventfd: initial value cannot be negative");
...
CAMLprim value core_linux_eventfd_read(value v_fd) {
  CAMLparam1(v_fd);
...
CAMLprim value core_linux_eventfd_write(value v_fd, value v_val) {
